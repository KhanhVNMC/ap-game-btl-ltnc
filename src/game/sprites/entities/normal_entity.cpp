...
void NormalEntity::moveSmooth(const int targetX, const int targetY, const function<void()> onComplete, const int speed_) {
    ...
    targetMoveY = targetY;
    this->speed = speed_;
    if (this->onMovedComplete != nullptr) {
        this->onMovedComplete(); // failsafe
    }
...
void NormalEntity::attackPlayer() {
    // prerequisite
    if (this->isAttacking || this->isSpawning) return;
    TetrisPlayer* target = this->pTetrisPlayer;

    const auto curLoc = target->getLocation();
    const auto toReturn = this->getLocation();
    const auto lastLane = target->currentLane;
